...
function UpdateAdministrativeTicket() {
  ...
  const fetchTicketDataById = async (ticketId) => {
    try {
      ...
      const response = await AdministrativeService.getTicket(ticketId);
      if (response) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await AdministrativeService.updateTicket(ticketData);
      navigate("/administrativehighpriority");
    } catch (error) {
...
export default UpdateAdministrativeTicket;