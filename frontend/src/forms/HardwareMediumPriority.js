import HardwareITService from '../services/HardwareITService'
import {  Link } from 'react-router-dom';
import { useState } from 'react';
import {  useEffect } from 'react';
import 'bootstrap/dist/css/bootstrap.min.css';
const  HardwareMediumPriority= () => {
...
useEffect(() => {
      ...
      const fetchTickets = () => {
        HardwareITService.mediumPriority()
          .then(response => {
            setTickets(response);
      ...
      const fetchTicket = async (ticketId) => {
        try {
          const ticket = await HardwareITService.getTicket(ticketId);
              ...
              {tickets.map(ticket => (
                <tr key={ticket.ticketId}>
                  ...
                  <td>
                    ...
                    <button className="btn btn-default"><Link to={`/UpdateHardwareTickets/${ticket.ticketId}`}> Update </Link></button>
                  </td>
                </tr>
              ))}
            </tbody>
    ...
    export default HardwareMediumPriority;
