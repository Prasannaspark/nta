...
function UpdateHardwareTicket() {
  ...
  const fetchTicketDataById = async (ticketId) => {
    try {
      ...
      const response = await HardwareITService.getTicket(ticketId);
      if (response) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await HardwareITService.updateHardwareTicket (ticketData);
      navigate("/hardwarehighpriority");
    } catch (error) {
...
export default UpdateHardwareTicket;