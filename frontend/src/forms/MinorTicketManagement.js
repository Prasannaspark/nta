...
import './MinorTicketManagement.css';

const  MinorTicketManagement= () => {
  ...
  const fetchTickets = () => {
    AvailableITService.viewMinorTicket()
      .then(response => {
        setTickets(response);
          ...
          {tickets.map(ticket => (
            <tr key={ticket.ticketId}>
              ...
              <td>
                ...
                <button className="btn btn-default"><Link to={`/update-minor-ticket/${ticket.ticketId}`}> Update </Link></button>
              </td>
            </tr>
          ))}
        </tbody>
...
export default MinorTicketManagement;